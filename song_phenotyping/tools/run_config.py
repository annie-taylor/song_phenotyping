...
:class:`RunConfig` captures the full parameter set for one pipeline run
(spectrogram generation through phenotyping) so that any output file can be
traced back to the exact inputs and settings that produced it.

:class:`RunRegistry` is a thin SQLite wrapper for storing and querying
:class:`RunConfig` records across runs.
...
def _make_serializable(obj):
    """Recursively convert non-JSON-serialisable types to serialisable form.

    Currently handles :class:`range` objects by converting them to a dict
    with a ``__range__`` marker key.
...
def _restore_serialized(obj):
    """Reverse :func:`_make_serializable`, reconstructing ``range`` objects.

    Parameters
    ----------
    obj : object
        Object as returned by :func:`json.loads`.

    Returns
    -------
    object
        Original Python object with ``range`` instances restored.
    """
    if isinstance(obj, dict):
...
# ---------------------------------------------------------------------------
# Lazy sub-config imports (avoids circular dependencies during migration)
# ---------------------------------------------------------------------------

def _import_umap_params():
    try:
        from song_phenotyping.embedding import UMAPParams
    except ImportError:
...
class RunConfig:
    ...
    One :class:`RunConfig` is created at the start of a run (or reconstructed
    from a saved JSON file). The :attr:`run_id` is intended to be embedded in
    every HDF5/pickle output file so results can always be traced back to the
    exact settings that produced them.
    ...
    created_at : str
        ISO 8601 timestamp (UTC) of when the run was created.
    spec_mode : str
        ``"syllable"`` for Stage A syllable-based extraction or ``"slice"``
        for Stage A1 fixed-window extraction.
    bird_ids : list of str
        Bird identifiers processed in this run.
    spec_params : dict
        Serialised :class:`~song_phenotyping.tools.spectrogram_configs.SpectrogramParams`.
    umap_params : dict
        Serialised UMAP parameter object.
    hdbscan_params : dict
        Serialised :class:`~song_phenotyping.labelling.HDBSCANParams` (winning
        configuration after grid search).
    phenotype_params : dict
        Serialised :class:`~song_phenotyping.phenotyping.PhenotypingConfig`.
    notes : str, optional
        Free-text annotation. Default is ``""``.

    See Also
    --------
    RunConfig.create : Factory method that accepts typed config objects.
    RunRegistry : SQLite store for querying runs across parameter settings.
    """
    ...
    def create(
        ...
        """Create a new :class:`RunConfig` with a fresh UUID and timestamp.
        ...
        spec_mode : str
            ``"syllable"`` or ``"slice"``.
        bird_ids : list of str
            Birds to associate with this run.
        spec_params : SpectrogramParams, optional
            Defaults to :class:`~song_phenotyping.tools.spectrogram_configs.SpectrogramParams`.
        umap_params : UMAPParams, optional
            Defaults to ``UMAPParams()``.
        hdbscan_params : HDBSCANParams, optional
            Defaults to ``HDBSCANParams()``.
        phenotype_params : PhenotypingConfig, optional
            Defaults to ``PhenotypingConfig()``.
        notes : str, optional
        ...
        -------
        RunConfig
            New instance with a generated :attr:`run_id`.
        """
    ...
    def spec_params_obj(self) -> SpectrogramParams:
        """Return :attr:`spec_params` as a :class:`SpectrogramParams` instance."""
    ...
    def umap_params_obj(self):
        """Return :attr:`umap_params` as a ``UMAPParams`` instance."""
        UMAPParams = _import_umap_params()
        return UMAPParams.from_dict(self.umap_params)

    @property
    def hdbscan_params_obj(self):
        """Return :attr:`hdbscan_params` as a ``HDBSCANParams`` instance."""
        HDBSCANParams = _import_hdbscan_params()
        return HDBSCANParams.from_dict(self.hdbscan_params)

    @property
    def phenotype_params_obj(self):
        """Return :attr:`phenotype_params` as a ``PhenotypingConfig`` instance."""
        PhenotypingConfig = _import_phenotyping_config()
    ...
    def to_dict(self) -> dict:
        ...
        -------
        dict
            All :class:`RunConfig` fields as plain Python types.
        """
        return {
            "run_id": self.run_id,
    ...
    def save(self, path: str):
        """Serialise this :class:`RunConfig` to a JSON file.
    ...
    def load(cls, path: str) -> "RunConfig":
        """Deserialise a :class:`RunConfig` from a JSON file.
...
class RunRegistry:
    """SQLite-backed store for :class:`RunConfig` records.
    ...
    def register(self, config: RunConfig):
        """Insert a :class:`RunConfig`. Silently replaces if *run_id* exists.
        ...
        with self._connect() as conn:
            conn.execute(
                """INSERT OR REPLACE INTO runs
                   ...
                   VALUES (?,?,?,?,?,?,?,?,?)""",
                (
                    row["run_id"], row["created_at"], row["spec_mode"],
                    json.dumps(row["bird_ids"]), json.dumps(row["spec_params"]),
                    json.dumps(row["umap_params"]), json.dumps(row["hdbscan_params"]),
                    json.dumps(row["phenotype_params"]), row["notes"],
                ),
            )

    def get(self, run_id: str) -> Optional[RunConfig]:
        """Retrieve a single :class:`RunConfig` by *run_id*.
    ...
    def query(self, bird_id: Optional[str] = None, **kwargs) -> pd.DataFrame:
        """Return a :class:`~pandas.DataFrame` of matching runs.

        Parameters
        ----------
        bird_id : str, optional
            Filter to runs whose *bird_ids* list contains this identifier
            (substring match on the serialised JSON).
        **kwargs
            Exact-match filters on top-level columns: ``spec_mode``,
            ``run_id``, ``notes``. To filter on nested parameters (e.g.
            ``n_neighbors=20``), filter the returned DataFrame directly::
        ...
        -------
        pandas.DataFrame
            One row per matching run, ordered by ``created_at`` descending.
            The ``spec_params``, ``umap_params``, ``hdbscan_params``, and
            ``phenotype_params`` columns contain plain dicts.
        """
        clauses, values = [], []
    ...
    def all(self) -> pd.DataFrame:
        """Return all registered runs as a :class:`~pandas.DataFrame`.
