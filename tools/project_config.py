"""
ProjectConfig — machine-local path configuration for the song phenotyping pipeline.

Reads config.yaml from the project root (or a path you specify). Each machine
keeps its own config.yaml (gitignored); config.yaml.example is committed as a
template.
