"""
HTML-based song catalogs for the song phenotyping pipeline.

Replaces the ReportLab PDF approach with matplotlib + base64 PNG embedded in HTML.
Produces two separate catalog types:

  1. Song catalog   — one continuous spectrogram per song, with manual labels
                      above and auto labels below the spectrogram, sorted
                      chronologically.

  2. Syllable-type catalog — for each unique label, a grid of all individual
                      syllable spectrograms of that type, drawn directly from
                      the pre-computed HDF5 arrays (no audio file needed).

Usage::

    from song_catalog_html import generate_song_catalog, generate_syllable_type_catalog

    # Continuous song view, using clustering rank 0
    html_path = generate_song_catalog(bird_path, rank=0, n_songs=30)

    # Per-label syllable grid
    html_path = generate_syllable_type_catalog(bird_path, rank=0, n_per_type=30)
"""
...
from tools.label_handler import LabelHandler, LabelType
...
def generate_song_catalog(
    ...
    Each entry shows a continuous-song spectrogram (``config.song_duration``
    seconds) with manual labels above and automated cluster labels below.
    Songs are sorted chronologically.

    Returns the path to the generated HTML file, or '' on failure.
    """
...
def generate_syllable_type_catalog(
    bird_path: str,
    rank: int = 0,
    label_source: str = 'auto',   # 'auto' | 'manual'
    config: Optional[CatalogConfig] = None,
) -> str:
    """
    Generate an HTML syllable-type catalog for one bird.

    For each unique label, shows a grid of up to ``config.n_per_type``
    individual syllable spectrograms drawn directly from the HDF5 arrays
    ...
    bird_path : str
    rank : int
        Clustering rank to use for auto labels.
    label_source : 'auto' | 'manual'
        Which label set to group by.
...
def generate_all_catalogs(
    ...
    """
    Generate song catalog + syllable-type catalogs (auto and manual if available).

    Returns a dict mapping catalog type to output path.
    """
    cfg = config or CatalogConfig()
    results = {}
